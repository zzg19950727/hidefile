...
BOOLEAN Ps_IsMemFS()
{
	CHAR cur_process[1024];

	RtlZeroMemory(cur_process, 1024);
	Ps_GetProcessName(cur_process, NULL);
	return TRUE;
	if (!_stricmp(cur_process, "memfs-x64.exe") ||
		!_stricmp(cur_process, "memfs-x86.exe") )
		return TRUE;
	else
		return FALSE;
}
